{config, pkgs, lib, modulesPath, ...}: {
  ...
  system.build.googleDeployImage = let
    ...
    toplevelHash = builtins.head (lib.splitString "-" (builtins.baseNameOf (builtins.unsafeDiscardStringContext config.system.build.toplevel)));
    toplevelVersion = (builtins.parseDrvName config.system.build.toplevel.name).version;
    imageName = lib.replaceStrings ["_" "."] ["-" "-"] "nixos-${toplevelHash}-${config.networking.hostName}";
