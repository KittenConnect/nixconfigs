{
  ...
  genFWMarkStr = (
    mark:
      {
        "string" = assert assertMsg (hasPrefix "0x" mark) "fwMark is string but does not start with 0x is it an int ?"; mark;
  ...
  mkWireguardConf = name: peer: let
    fwMarkString = (
      let
        mark =
          if peer.fwMark != null
          then peer.fwMark
          else if peer.onIFACE != null
          then peer.port
          else null;
      in
        genFWMarkStr mark
