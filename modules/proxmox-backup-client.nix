...
in
{
  options.services.proxmox-backup-client.backups = mkOption {
    description = ''
      Periodic backups to create with Proxmox Backup Client.
    '';
    type = types.attrsOf (
      types.submodule (
        { name, ... }:
        {
          options = {
            passwordFile = mkOption {
              ...
              '';
              example = "/etc/nixos/proxmox-backup-client-password";
            };
            ...
            exclude = mkOption {
              ...
              description = ''
                ...
                https://proxmox-backup-client.readthedocs.io/en/latest/040_backup.html#excluding-files for
                details on syntax.
              '';
              example = [
                "/var/cache"
            ...
            pruneOpts = mkOption {
              ...
              default = [ ];
              description = ''
                A list of options (--keep-\* et al.) for 'proxmox-backup-client forget
    ...
    example = {
      remotebackup = {
        ...
        passwordFile = "/etc/nixos/secrets/proxmox-backup-client-password";
        extraOptions = [
  ...
  config = {
    assertions = mapAttrsToList (n: v: {
      ...
      message = "services.proxmox-backup-client.backups.${n}: exactly one of repository or repositoryFile should be set";
    }) config.services.proxmox-backup-client.backups;
    systemd.services = mapAttrs' (
      name: backup:
      let
        extraOptions = concatMapStrings (arg: " -o ${arg}") backup.extraOptions;
        pbcCmd = "${backup.package}/bin/proxmox-backup-client${extraOptions}";
        excludeFlags = optional (
          backup.exclude != [ ]
        ) "--exclude-file=${pkgs.writeText "exclude-patterns" (concatStringsSep "\n" backup.exclude)}";
        filesFromTmpFile = "/run/proxmox-backup-client-backups-${name}/includes";
        doBackup = (backup.dynamicFilesFrom != null) || (backup.paths != null && backup.paths != [ ]);
        pruneCmd = optionals (builtins.length backup.pruneOpts > 0) [
          (pbcCmd + " forget --prune " + (concatStringsSep " " backup.pruneOpts))
        ];
        checkCmd = optionals backup.runCheck [
          (pbcCmd + " check " + (concatStringsSep " " backup.checkOpts))
        ];
      ...
      in
      nameValuePair "proxmox-backup-client-backups-${name}" (
        {
          environment =
            {
              # not %C, because that wouldn't work in the wrapper script
              RESTIC_CACHE_DIR = "/var/cache/proxmox-backup-client-backups-${name}";
          ...
          serviceConfig = {
            ...
            ExecStart =
              ...
              ++ checkCmd;
            User = backup.user;
            RuntimeDirectory = "proxmox-backup-client-backups-${name}";
            CacheDirectory = "proxmox-backup-client-backups-${name}";
    ...
    systemd.timers =
      mapAttrs'
        ...
        (filterAttrs (_: backup: backup.timerConfig != null) config.services.proxmox-backup-client.backups);
    ...
    environment.systemPackages = lib.mapAttrsToList (
      ...
      pkgs.writeShellScriptBin "proxmox-backup-client-${name}" ''
        ...
        ${lib.pipe config.systemd.services."proxmox-backup-client-backups-${name}".environment [
          (lib.filterAttrs (n: v: v != null && n != "PATH"))
          (lib.mapAttrsToList (n: v: "${n}=${v}"))
          (lib.concatStringsSep "\n")
        ]}
        PATH=${config.systemd.services."proxmox-backup-client-backups-${name}".environment.PATH}:$PATH

        exec ${pbcCmd} $@
      ''
    ) (lib.filterAttrs (_: v: v.createWrapper) config.services.proxmox-backup-client.backups);
  };
}
