...
let
  ...
  autologinArg = lib.optionalString (cfg.autologinUser != null) "-a ${cfg.autologinUser}";
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    systemd.units."kmsconvt@.service".text = ''
      ...
      [Service]
      ExecStart=
      ExecStart=${pkgs.kmscon}/bin/kmscon "--vt=%I" ${cfg.extraOptions} --seats=seat0 --no-switchvt --configdir ${configDir} --login -- ${pkgs.util-linux}/sbin/agetty --login-program ${pkgs.shadow}/bin/login ${autologinArg} -o '-p -- \\u' - xterm-256color
